...
//每个follower节点对应的NextIndex值和MatchIndex值都封装在Progress实例中
//除此之外，每个Progress实例中还封装了对应follower节点的相关信息
type Progress struct {
	...
	//从当前Leader节点的角度来看，该Progress实例对应的
	//Follower节点是否存活
	RecentActive bool
...
func (pr *Progress) MaybeUpdate(n uint64) bool {
	var updated bool
	if pr.Match < n {
		//n之前的成功发送所有的entry记录已经写入对应节点的raftLog中
		pr.Match = n
		updated = true
		pr.ProbeAcked()
	}
